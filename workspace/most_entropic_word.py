...
guesses1 = load_dictionary(guesses_dictionary)


#FIRST HEURISTIC: entropy
def heuristic_entropy(guesses1, guesses):
    entropy_list = {}
    max = -1
    max_word = ""
    #print(len(possible_solutions))
    if (len(guesses1) == 1): return guesses1[0]
    else:
        progress_bar = tqdm(total=len(guesses), desc="Progress")
        for word in guesses:
            sum = sum_entropy(word, guesses1)
...
result = heuristic_entropy(guesses1, guesses)
print(result)